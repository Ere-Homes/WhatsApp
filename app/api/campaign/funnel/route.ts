...
export async function GET() {
  try {
    ...
    const agg: Record<string, { sent: number; delivered: number; read: number; failed: number }> = {};
    ...
    for (let from = 0; ; from += 1000) {
      ...
      for (const m of (data as any[]) || []) {
        const a = (agg[m.campaign] ||= { sent: 0, delivered: 0, read: 0, failed: 0 });
        const s = m.status;
        if (s === "failed" || s === "undelivered") a.failed++;
        else {
